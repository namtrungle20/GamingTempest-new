...
import useCatalogManage from '@/hook/admin/useCatalogManage'
...
const SanPhamManagePage = () => {
    ...
    const thuongHieuHook = useCatalogManage(brandService, Brand)
    const loaiSanPhamHook = useCatalogManage(categoryService, Category)
    ...
    return (
        <Mui.Box>
            ...
            {tab === 0 && (
                <Mui.Box>
                    <Mui.Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
                        ...
                        <Mui.Button
                            variant="contained" startIcon={<Icon.Add />}
                            onClick={openCreate}
                                    ...
                                    {loading ? (
                                        ...
                                        <Mui.TableRow>
                                            <Mui.TableCell colSpan={6} align="center" sx={{ py: 6 }}>
                                                <Mui.Typography color="text.secondary">Không có sản phẩm nào</Mui.Typography>
                                            </Mui.TableCell>
                                        </Mui.TableRow>
                                    ) : sanPhams.map((sp) => (
                                        <Mui.TableRow key={sp.id} hover>
                                            <Mui.TableCell>
                                                <Mui.Box display="flex" alignItems="center" gap={2}>
                                                    <Mui.Avatar src={sp.imageUrl} variant="rounded" sx={{ width: 48, height: 48 }}>
                                                        <Icon.Inventory2 />
                                                    </Mui.Avatar>
                                                    <Mui.Box>
                                                        <Mui.Typography variant="body2" fontWeight={600}>{sp.name}</Mui.Typography>
                                                        <Mui.Typography variant="caption" color="text.secondary" noWrap sx={{ maxWidth: 200, display: 'block' }}>
                                                            {sp.mota || '—'}
                                                        </Mui.Typography>
                                                    </Mui.Box>
                                                </Mui.Box>
                                            ...
                                            <Mui.TableCell>
                                                <Mui.Chip label={sp.soluong} size="small" color={sp.stockStatus} />
                                            </Mui.TableCell>
                                            <Mui.TableCell align="right">
                                                <Mui.IconButton size="small" onClick={() => openEdit(sp)} color="primary">
                                                    <Icon.EditOutlined fontSize="small" />
                                                </Mui.IconButton>
                                                <Mui.IconButton size="small" onClick={() => openDeleteDialog(sp.id, sp.name)} color="error">
                    ...
                    <Mui.Modal open={modalOpen} onClose={closeModal} closeAfterTransition>
                        <Mui.Box sx={{
                            ...
                            outline: 'none', borderTop: '5px solid', borderColor: 'primary.main',
                            ...
                            <Mui.Stack spacing={2.5}>
                                <Mui.TextField
                                    fullWidth label="Tên sản phẩm" required
                                    value={form.name}
                                ...
                                <Mui.Box display="flex" gap={2}>
                                    <Mui.TextField
                                        ...
                                        value={form.gia}
                                        onChange={(e) => handleFormChange('gia', e.target.value)}
                                    />
                                    <Mui.TextField
                                ...
                                <Mui.FormControl fullWidth>
                                    ...
                                    <Mui.Select
                                        value={form.loai_id}
                                        label="Loại sản phẩm"
                                        onChange={(e) => handleFormChange('loai_id', e.target.value)}
                                    >
                                        {categories.map(c => (
                                            <Mui.MenuItem key={c.loai_id} value={c.loai_id}>{c.name}</Mui.MenuItem>
                                        ))}
                                    </Mui.Select>
                                </Mui.FormControl>
                                <Mui.FormControl fullWidth>
                                    ...
                                    <Mui.Select
                                        ...
                                        label="Thương hiệu"
                                        onChange={(e) => handleFormChange('thuonghieu_id', e.target.value)}
                                    >
                                        {brands.map(b => (
                                            <Mui.MenuItem key={b.thuonghieu_id} value={b.thuonghieu_id}>{b.name}</Mui.MenuItem>
                                        ))}
                                    </Mui.Select>
                                </Mui.FormControl>
                                ...
                                >
                                    {form.image ? (
                                        <Mui.Box>
                                            ...
                                            <Mui.Typography variant="body2" color="primary.main" mt={1}>{form.image.name}</Mui.Typography>
                                        </Mui.Box>
                                    ) : (
                                        <Mui.Box>
                                            <Icon.CloudUpload color="disabled" sx={{ fontSize: 40 }} />
                                            <Mui.Typography variant="body2" color="text.secondary" mt={1}>
                                                {editTarget ? 'Chọn ảnh mới (tuỳ chọn)' : 'Chọn ảnh sản phẩm'}
                                            </Mui.Typography>
                                        </Mui.Box>
                                    )}
                                </Mui.Box>
                    ...
                    <Mui.Dialog open={deleteDialog.open} onClose={closeDeleteDialog}>
                        ...
                        <Mui.DialogContent>
                            <Mui.Typography>Bạn có chắc muốn xóa sản phẩm <strong>{deleteDialog.name}</strong>?</Mui.Typography>
                        </Mui.DialogContent>
                        <Mui.DialogActions>
                            <Mui.Button onClick={closeDeleteDialog}>Huỷ</Mui.Button>
                            <Mui.Button onClick={handleDelete} color="error" variant="contained" sx={{ fontWeight: 700 }}>Xóa</Mui.Button>
                        </Mui.DialogActions>
                    </Mui.Dialog>
            ...
            {/* Tab: Thương hiệu */}
            {tab === 1 && (
                <CatalogManageTab label="Thương hiệu" hook={thuongHieuHook} />
            )}

            {/* Tab: Loại sản phẩm */}
            {tab === 2 && (
                <CatalogManageTab label="Loại sản phẩm" hook={loaiSanPhamHook} />
            )}
        </Mui.Box>
    )
}
